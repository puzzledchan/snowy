#ifndef SNOWY_HELPER_H
#define SNOWY_HELPER_H
...
template <typename T> struct TryWrapper;
...
template <typename F, typename... Args> struct CanCallWith {
  ...
  template <typename Dummy> static constexpr std::false_type Check(...) {
...
template <typename F> struct CallableResult<F, void> {
  // Test F call with arg type: void or Try(void)
  typedef typename std::conditional<
      CanCallWith<F>::value, // if true, F can call with void
      ResultOfWrapper<F>,
      typename std::conditional<               // NO, F(void) is invalid
          CanCallWith<F, Try<void> &&>::value, // if true, F(Try<void>&&) is
                                               // valid
...
void CollectVariadicHelper(const std::shared_ptr<CTX<Ts...>> &ctx, THead &&head,
  ...
  head.Then([ctx](InnerTry &&t) {
    ctx->template SetPartialResult<InnerTry, sizeof...(Ts) - sizeof...(TTail) -
                                                 1>(std::move(t));
  });
